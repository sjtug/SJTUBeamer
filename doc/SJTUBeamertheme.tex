...
\texttt{navigation=tools|subsections}\\
\indent 导航栏选项。默认为 \verb"tools"，即工具栏与页码组合；可以设置为 \verb"subsections"，将会产生子章节的跳转进度条。

\texttt{lang=cn|en}\\
\indent 语言选项，主要影响校徽是中文校徽，还是英文校徽。而启用该选项为英文将会导致中文包不会被加载，因为推荐使用英文徽标时，幻灯片内容为英文。

\texttt{cjk=false|true}\\
\indent 是否使用 \textsf{CJK} 宏包。如果开启此选项，请使用 \verb"\begin{CJK}{UTF8}{hei}" 和 \verb"\end{CJK}" 包裹 \verb"document" 环境中的内容。
