...
\ProvidesFile{sjtubeamerdevguide.tex}[2026/05/20 v3.2.0 Development Guide for sjtubeamer (English)]
...
\changes{v3.0.0}{2022/11/25}{Major version release. Start tracking changes.}
...
\begin{titlepage}
  ...
  \hskip-9em\resizebox{1.02\paperwidth}{!}{\sjtugate[white]}\vskip2.3cm
\end{titlepage}
...
files (or the kernel). In details,
\begin{itemize}
  ...
  \item Zipping packages like \texttt{sjtubeamer-ctan.zip}, \texttt{sjtubeamer.tds.zip}
\end{itemize}

We use \verb"make" and related scripts (Section \ref{sec:make}) for Continuous
Integration on GitHub Actions. In details,
\begin{itemize}
...
\fcmd{l3build manifest} This will refresh the repository manifest file
\verb"MANIFEST.md". In the current version of \verb"l3build", you need
to run the following commands to make it properly refreshed:
\begin{verbatim}
  l3build ctan
  l3build manifest
  l3build ctan
...
\subsection{Continuous Integration}\label{sec:make}
...
\fcmd{make build-online} This command will build the online distribution in
\verb"build/online" and generate the zipped file at
\verb"./sjtubeamer-online.zip". See \verb"MANIFEST.md" for what it contains.

\fcmd{make format-dev} This command will format all the files, and clear the
indent. In order to improve the code quality. We use the following parameters
for \verb"latexindent" \cite{latexindent}:
\begin{verbatim}
  latexindent *.tex -l latexindent.yaml -w -c build -m -r
\end{verbatim}
where \verb"latexindent.yaml" regulates the indent is formatted in spaces, no
more than 80 characters per line and the links will not be text-wrapped.

\begin{warn}
  For MiK\TeX{} users, you could download
  \href{https://mirrors.sjtug.sjtu.edu.cn/ctan/support/latexindent.zip}{\ttfamily latexindent}
  and link the executable path \verb"latex-workshop.latexindent.path" in Visual Studio Code.
\end{warn}
...
Studio Code snippets run properly. You could obtain a manifest for what the
files are in \verb"MANIFEST.md".
...
This environment has one parameter to indicate the name of the macro. For
tidiness, There should be only one space after the beginning of the
environment.
...
generating the Visual Studio Code snippets, the description will be shown.
Since Visual Studio Code Snippets only support text with 40 characters per
line, you should keep the description in the same format. The searching on the
command in \verb"macrocode" environments will decide the parameter number and
the default parameter in the code snippets as well. If you want to add some
notes to this macro, add them before the end of the \verb"macro" environment.
You could use multiple \verb"macrocode" environments in order to add comments
...
listed below (or read \verb"MANIFEST.md" for more details).

If you want to install \themename{} manually, add these files into the
searching directory of \TeX{} and refresh the filename directory. This could be
achieved by \verb"l3build install" mentioned in Section \ref{sec:l3build}.
...
\begin{figure}[h]
  ...
  \parbox[t]{0.25\textwidth}{\centering $\downarrow$}\framebox[0.25\textwidth]
  {\ttfamily sjtucover.sty}\parbox[t]{0.25\textwidth}{\centering $\downarrow$}\parbox[t]{0.25\textwidth}{}
...
\verb"\definelogo". You need to specify the control sequence name and the file
path in the first two arguments.
...
\begin{figure}[h]
  \begin{tikzpicture}
    ...
    \node at (1.75,-1.5) {\verb"\definelogo"};
...
\themename{} makes its effort on engine support in the following ways:
\begin{enumerate}
...
\begin{tabular}{c|ccc}
  \TeX\ Live & 2019\textasciitilde 2026 & 2015\textasciitilde 2018 & \textasciitilde 2014
  \\
  \hline
  Test       & Supported                & Need compatibility fix   & Not supported
  \\
\end{tabular}
