...
To use a different cover style, use \verb"\setbeamertemplate{title page}[maxplus]" before calling \verb"\maketitle". The same as \verb"bottom page", \verb"part page", \verb"section page" and \verb"subsection page". For the sake of simplicity, the later three templates could be modified by one parent template \verb"sectioning pages".
...
\themename\ will show the section it belongs when typesetting the subsection page, so use \verb"sectioning block" helper template to achieve that. When you want to override the original \verb"part page" template, use \verb"\definesecpage{part}{min}" command.
...
This will redirect to the target \verb"sectioning page" by setting the correct configuration before using it. This might seems redundent since it has to set every time it uses. It seems that the code takes time but actually not, since it is macro programming for \LaTeX{} and the canonical conditional changes will only redirect the name of the macro instead of actually push the full data into the register. It is also beneficial for a unified management with the provided macros from \verb"beamer" class.

About numbering scheme, it uses internal variables like \verb"\sjtubeamer@cover@sectionnumber" to store that. To adapt to the \verb"ctexbeamer" class, once it is loaded, the macro will get redirect to the macros defined in \verb"ctexheading" (remember, if the package \verb"ctex" is loaded seperately, this package will not get loaded). As a result, user could use \verb"\ctexset" interface to modify the numbering style after the theme is loaded (since some modification to \verb"\ctexset" may be introduced in this theme). 
