...
为了避免使用额外的文献管理程序，可以使用手动输入的方式排印文献列表。

\beamerdemo[1]{step16+-.tex}

\begin{commentlist}
  \item \LaTeX{} 本身提供了 \texttt{thebibliography} 环境，环境的强制参数用于指定排版最长的标签（这里是两位数占位符），配套 \texttt{\textbackslash{}bibitem[标签]\{主键\}}，在 \texttt{beamer} 中需要辅以 \texttt{\textbackslash{}newblock} 来分隔作者、文章标题、书目与其他内容。
