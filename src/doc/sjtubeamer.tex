...
\begin{document}
    ...
    \section{何时应该使用这个主题？}
    众所周知，\LaTeX\ 是一款非常专业的排版语言，但是其编译速度却不是即时的。而 \texttt{beamer} 是为了方便 \LaTeX\ 用户制作幻灯片而创造的宏包，下面是可能需要使用 \texttt{beamer} 的时机：
    \begin{enumerate}
        \item 重要正式的学术场合。
        \item 大型文档的构建（上百页）。
    ...
    \section{何时不应该使用这个主题？}
    但遗憾的是，\LaTeX\ 不是可见即可得的，编译需要花费时间。学习 \LaTeX\ 也需要花费时间。下面是不推荐使用的情况：
    \begin{enumerate}
